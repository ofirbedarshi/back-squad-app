...
function CoordinateInput({ value, onChange, hasError = false, disabled = false }: CoordinateInputProps) {
  ...
  const palachInputRef = useRef<HTMLInputElement | null>(null)

  const east = value?.east ?? ''
  const north = value?.north ?? '3'
  const checkDigit = north.slice(0, 1) || '3'
  const northMain = north.slice(1, 7)
  const palach = value?.palach ?? '36'

  function updateCoordinates(nextEast: string, nextNorthMain: string, nextCheckDigit: string, nextPalach: string) {
    onChange({
      east: nextEast,
  ...
  function handleEastChange(rawValue: string) {
    ...
    updateCoordinates(nextEast, northMain, checkDigit, palach)
  }

  function handleNorthMainChange(rawValue: string) {
    const nextNorthMain = rawValue.replace(/\D/g, '').slice(0, 6)
    updateCoordinates(east, nextNorthMain, checkDigit, palach)
  }

  function handleCheckDigitChange(rawValue: string) {
    const nextCheckDigit = rawValue.replace(/\D/g, '').slice(0, 1) || '3'
    updateCoordinates(east, northMain, nextCheckDigit, palach)
  }

  function handlePalachChange(rawValue: string) {
    const nextPalach = rawValue.replace(/\D/g, '').slice(0, 2) || '36'
    updateCoordinates(east, northMain, checkDigit, nextPalach)
  }
      ...
      <div className="flex items-center px-2 py-2 gap-1">
        ...
        <div className="shrink-0">
          ...
          <input
            ...
            maxLength={2}
            value={palach}
            onChange={(event) => handlePalachChange(event.target.value)}
            onFocus={handleSelectOnFocus}
            onClick={handleSelectOnFocus}
            disabled={disabled}
