...
import ObstacleTargetDisplay from './ObstacleTargetDisplay'
import { degreeOpts } from './positionForm.schema'
import type { PositionFormValues } from './positionForm.schema'

type ObstacleErrors = Array<{ compass?: { message?: string } } | undefined>
...
function ObstaclesCard() {
  ...
  const errorMessage = obstacleErrors?.flatMap((e) => [e?.compass?.message]).find(Boolean)
        ...
        >
          ...
          {([0, 1, 2] as const).map((i) => (
            <Fragment key={i}>
              ...
              />
              <ObstacleTargetDisplay compass={obstacles?.[i]?.compass} />
            </Fragment>
          ))}
        </div>
        {errorMessage && (
